...
```{r filter1}
SamaresEq_readr %>% filter( Surface > 0.75) 
SamaresEq_readr %>% filter( Site == 3) 

Grand_samares <- SamaresEq_readr %>% filter( Surface > 0.75) 
Grand_samares %>% print(n=10)
```
...
```{r biomass-sol, eval = FALSE, echo = FALSE}
...
biomass_france_post2005 <- biomass_dta %>% 
  filter(COUNTRY == 'FRANCE', YEAR >= 2005) 
 biomass_dta %>% 
   group_by(COUNTRY) %>% 
...
`r chunk_reveal("biomass-sol", break_type = "auto")`
